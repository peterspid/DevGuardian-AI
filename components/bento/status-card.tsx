...
const REGIONS = [
  { name: "PLANNER", status: "VERIFIED", latency: "repo.read" },
  { name: "CODE", status: "SCOPED", latency: "repo.write" },
  { name: "SECURITY", status: "VERIFIED", latency: "scan" },
  { name: "DEPLOY", status: "BLOCKED", latency: "release" },
]
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col p-4 gap-0">
        ...
        <div className="grid grid-cols-3 gap-2 border-b border-border pb-2 mb-2">
          ...
          <span className="text-[9px] tracking-[0.15em] uppercase text-muted-foreground text-right">Scope</span>
        </div>
        {REGIONS.map((region) => (
          <div
            key={region.name}
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span
                ...
                style={{
                  backgroundColor: region.status === "BLOCKED" ? "hsl(var(--muted-foreground))" : "#ea580c",
                }}
              />
