...
function StatusLine() {
  ...
  return (
    <div className="flex items-center gap-2 text-[10px] tracking-widest text-muted-foreground uppercase font-mono">
      ...
      <span>live agent throughput: {throughput} actions/min</span>
    </div>
  )
}
...
const TIERS: Tier[] = [
  {
    ...
    period: "/ demo",
    tag: null,
    description: "Read-only repo inspection with Terminal3 status and audit trail preview.",
    features: [
      { text: "Repository Explorer", included: true },
      { text: "Planner Agent", included: true },
      { text: "Security snapshot", included: true },
      { text: "Local audit fallback", included: true },
      { text: "Production deploy gate", included: false },
      { text: "Live Terminal3 execution", included: false },
    ],
    cta: "OPEN DASHBOARD",
    highlighted: false,
  },
  {
    ...
    period: " agents",
    tag: "HACKATHON",
    description: "Full multi-agent workflow with Terminal3 permission witnesses and review gates.",
    features: [
      { text: "Planner, Code, Security, Test", included: true },
      { text: "Review and Deploy Agents", included: true },
      { text: "Delegation credential witness", included: true },
      { text: "Signed protected actions", included: true },
      { text: "Mongo-backed audit logs", included: true },
      { text: "Human deploy approval", included: true },
    ],
    cta: "RUN WORKFLOW",
    highlighted: true,
  },
  {
    ...
    period: "",
    tag: null,
    description: "Production deployment mode for Vercel, Render, and GitHub release provenance.",
    features: [
      { text: "Vercel frontend target", included: true },
      { text: "Render backend target", included: true },
      { text: "GitHub release evidence", included: true },
      { text: "Terminal3 deploy.release", included: true },
      { text: "Approval audit event", included: true },
      { text: "Custom tenant contract", included: true },
    ],
    cta: "VERIFY DEPLOY",
    highlighted: false,
  },
...
export function PricingSection() {
  return (
    <section className="w-full px-6 py-20 lg:px-12">
      ...
      >
        <div className="flex flex-col gap-3">
          <h2 className="text-2xl lg:text-3xl font-mono font-bold tracking-tight uppercase text-foreground text-balance">
            Select an agent operating mode
          </h2>
          <p className="text-xs lg:text-sm font-mono text-muted-foreground leading-relaxed max-w-md">
            Each mode keeps agent identity, permission scope, and production release evidence visible.
          </p>
        </div>
        <StatusLine />
      </motion.div>
      ...
      >
        <span className="text-[10px] tracking-[0.2em] uppercase text-muted-foreground font-mono">
          {"* DevGuardian runs in demo mode without secrets and switches to live Terminal3/OpenAI/MongoDB when runtime env vars are configured."}
        </span>
        <div className="flex-1 border-t border-border" />
      </motion.div>
    </section>
